                  ...
                  {card.sections.map((section, idx) => {
                    ...
                    return (
                      <div key={section.id} className="rounded border bg-background p-3 space-y-1">
                        ...
                        <div className="text-xs text-foreground/70 prose prose-xs dark:prose-invert max-w-none line-clamp-4 card-prose" dangerouslySetInnerHTML={{ __html: sanitizeHtml(section.content) }} />
                      </div>
                    );
                  })}
                </div>
