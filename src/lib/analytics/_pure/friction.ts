...
export function calcStressPerformance(
  reviewLog: ReviewLogEntry[],
  latencyLog: LatencyEntry[],
): StressPerformance | null {
  ...
  const normalGrades: number[] = [];
  const stressGrades: number[] = [];
  ...
  reviewLog.forEach(r => {
    ...
    if (matching.latencyMs < stressThreshold) stressGrades.push(r.grade);
    else normalGrades.push(r.grade);
  });
